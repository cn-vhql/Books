...
func writeRuntimeConfig() error {
	...
	configPath := filepath.Join(staticDir, "config.js")
	content := fmt.Sprintf(
		"window.__KOODO_SERVER_CONFIG__ = {\n  libraryUsername: %q,\n  libraryPassword: %q\n};\n",
...
func requiresAuth(r *http.Request) bool {
	...
	case strings.HasPrefix(path, "/api/library/"):
		return false
	case path == "/upload", path == "/download", path == "/delete", path == "/list":
		return true
...
func handler(w http.ResponseWriter, r *http.Request) {
	...
	// Basic Auth for API-like endpoints only. Static assets and SPA routes stay public,
	// otherwise the browser repeatedly challenges for icon/font/script requests.
	if requiresAuth(r) && !authenticate(r) {
	...
	switch {
	case libraryEnabled && strings.HasPrefix(path, "/api/library/"):
		libraryHandler(w, r)
...
func main() {
	...
	if staticDir != "" {
		if _, err := fs.Stat(os.DirFS(staticDir), "."); err != nil {
			log.Fatalf("Cannot access static directory %s: %v", staticDir, err)
		}
		if err := writeRuntimeConfig(); err != nil {
			log.Fatalf("Cannot write runtime config: %v", err)
		}
	}
	if libraryEnabled {
		if err := ensureLibrarySchema(); err != nil {
			log.Fatalf("Cannot initialize library schema: %v", err)
		}
	}
