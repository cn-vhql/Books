...
func openBooksReadOnlyDB() (*sql.DB, error) {
	dbPath := filepath.Join(uploadDir, "config", "books.db")
	return sql.Open("sqlite", dbPath+"?_pragma=busy_timeout(5000)&_pragma=journal_mode(WAL)&mode=ro")
}
