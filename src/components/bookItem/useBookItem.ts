...
export function useBookItem(props: BookItemSharedProps) {
  ...
  const normalizeCoverUrl = useCallback(
    (url: string, bustCache: boolean = false) => {
      if (!url) {
        return "";
      }
      if (
  ...
  useEffect(() => {
    ...
    async function loadCoverAndOffline() {
      if (isServerMode) {
        const newCover = normalizeCoverUrl(await CoverUtil.getCover(book));
        if (!cancelled) {
          setCover(newCover);
          setIsCoverExist(!!book.cover);
      ...
      if (!cancelled) {
        setCover(normalizeCoverUrl(newCover));
  ...
  }, [book, isServerMode, normalizeCoverUrl]);
  ...
  useEffect(() => {
    ...
    async function refreshCover() {
      if (isServerMode) {
        const newCover = normalizeCoverUrl(
          await CoverUtil.getCover(book),
          refreshTriggered
        );
        if (!cancelled) {
          setIsFavorite(
            ConfigService.getAllListConfig("favoriteBooks").indexOf(book.key) >
              -1
          );
          setCover(newCover);
          setIsCoverExist(!!book.cover);
    ...
    normalizeCoverUrl,
    handleRefreshBookCover,
  ]); // intentionally omitting most props to replicate componentWillReceiveProps behaviour
  ...
  const handleJump = useCallback(
    (event?: React.MouseEvent) => {
      ...
      if (isServerMode && handleDetailDialog) {
