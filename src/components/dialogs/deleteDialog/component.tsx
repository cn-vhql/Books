...
> {
  ...
  handleComfirm = async () => {
    ...
    if (ServerLibrary.isEnabled()) {
      ...
      await this.deleteBooksImmediately();
    } else {
      this.deleteBooks();
    }
  ...
  deleteBooksImmediately = async () => {
    const keys = this.props.isSelectBook
      ? this.props.selectedBooks
      : [this.props.currentBook.key];
    for (const key of keys) {
      const current =
        this.props.books.find((item) => item.key === key) ||
        this.props.deletedBooks.find((item) => item.key === key);
      await this.deleteBook(key, current?.format?.toLowerCase() || "epub");
    }
    this.props.handleSelectedBooks([]);
    this.props.handleFetchBooks();
    this.props.handleFetchBookmarks();
  ...
  deleteServerBooks = async () => {
    const keys = this.props.isSelectBook
      ? this.props.selectedBooks
      : [this.props.currentBook.key];
    for (const key of keys) {
      const current = this.props.books.find((item) => item.key === key);
      await this.deleteBook(key, current?.format?.toLowerCase() || "epub");
    }
    this.props.handleSelectedBooks([]);
    this.props.handleSelectBook(false);
  ...
  deleteBook = (key: string, format: string) => {
    return new Promise<void>(async (resolve, reject) => {
      ...
      DatabaseService.deleteRecord(key, "books")
        ...
        .catch((err) => {
          toast.error("删除图书失败: " + err.message, {
            id: "delete-books",
          });
          console.error(err);
          reject(err);
