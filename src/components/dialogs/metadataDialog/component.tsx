...
> {
  ...
  handleSearch = async () => {
    ...
    if (!ServerLibrary.isEnabled() && !this.props.isAuthed) {
    ...
    try {
      const res = ServerLibrary.isEnabled()
        ? await ServerLibrary.searchMetadata(searchName, searchAuthor)
        : await getBookMetadata(searchName, searchAuthor);
