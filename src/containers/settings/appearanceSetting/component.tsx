...
> {
  ...
  renderToggleRow = (title: string, propName: string, desc?: string) => {
    return (
      <SettingFormRow
        key={propName}
        title={<Trans>{title}</Trans>}
        desc={desc ? <Trans>{desc}</Trans> : undefined}
        control={
          <span
            className="single-control-switch"
            onClick={() => {
              this.handleSetting(propName);
            }}
            style={this.state[propName] ? {} : { opacity: 0.6 }}
          >
            <span
  ...
  renderSwitchOption = (optionList: any[]) => {
    return optionList.map((item) => {
      ...
      return this.renderToggleRow(item.title, item.propName, item.desc);
    });
  };

  render() {
    const isServerMode = !!this.props.isServerMode;
    const appearanceOptions = isServerMode
      ? appearanceSettingList
      : appearanceSettingList;

    return (
      <SettingFormList>
        {this.renderSwitchOption(appearanceOptions)}
        {!isServerMode && (
          <SettingFormRow
            title={<Trans>System font</Trans>}
            control={
              <select
                name=""
        ...
        <SettingFormSection title={<Trans>Theme color</Trans>}>
        ...
        {!isServerMode && (
          <SettingFormSection title={<Trans>TTS highlight style</Trans>}>
            {this.renderTtsHighlightSetting()}
          </SettingFormSection>
        )}
        {!isServerMode && (
          <SettingFormSection title={<Trans>Search highlight style</Trans>}>
            {this.renderSearchHighlightSetting()}
          </SettingFormSection>
        )}
        <SettingFormSection title={<Trans>Appearance</Trans>}>
        ...
        {!isServerMode && (
          <>
            <SettingFormRow
              title={<Trans>Custom app style</Trans>}
              desc={
                <Trans>
