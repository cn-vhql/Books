...
class Reader extends React.Component<ReaderProps, ReaderState> {
  ...
  async UNSAFE_componentWillMount() {
    ...
    DatabaseService.getRecord(key, "books").then(async (book: Book | null) => {
      if (!book && ServerLibrary.isEnabled()) {
        try {
          const response = await ServerLibrary.getBook(key);
          book = response.book;
        } catch (error) {
          console.error("Failed to load server book details:", error);
        }
      }
  ...
  render() {
    ...
    return (
      <div className="viewer">
        ...
        {!ServerLibrary.isEnabled() && <SupportDialog />}
