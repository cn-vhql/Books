...
export const initTheme = () => {
  ...
  if (!isElectron && !ConfigService.getReaderConfig("libraryMode")) {
    ConfigService.setReaderConfig("libraryMode", "server");
  }
  if (!isElectron && window.__KOODO_SERVER_CONFIG__?.libraryUsername) {
    ConfigService.setReaderConfig(
      "libraryUsername",
      window.__KOODO_SERVER_CONFIG__.libraryUsername
    );
  }
  if (!isElectron && window.__KOODO_SERVER_CONFIG__?.libraryPassword) {
    ConfigService.setReaderConfig(
      "libraryPassword",
      window.__KOODO_SERVER_CONFIG__.libraryPassword
    );
  }
